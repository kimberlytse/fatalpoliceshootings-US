...
author: "Arbaaz Mohideen & Kimberly Tse - \"Partners in Crime\"  <br> Research Mentor: Dr. Sayed Mostafa" 
affiliation: "North Carolina A&T State University, St. Mary's University"
date: "7/31/2020"
output: 
  html_document:
    toc: true
...
county_level_model <- read_csv("finalData/county_level_model.csv")
```
...
library(knitr)
state.summary.rate = read_csv("finalData/summary-state-rank.csv")
state.summary.rate = state.summary.rate%>% 
  na.omit()
kable(state.summary.rate, caption = "Table 1: US States Ranked by Total # Shootings per 100k (Shooting Rate) over the period of Jan 2015 - June 2020")
```


```{r Table 2, echo=F, message = F}
state.summary.vars.quant = read_csv("finalData/state-summary-vars-quantitative.csv")
kable(state.summary.vars.quant, caption = "Table 2: State Level Summary Statistics for Quantitative Variables  [Mean (SD) of Annual Values]")
```


```{r Table 3, echo=F, message = F}
state.summary.vars.cat = read_csv("finalData/state-summary-vars-categorical.csv")
...
state_level_model%>%
  ...
  labs(x="Population Diversity",y="Shootings per 100k", title = "Shootings per 100k vs Population Diversity 2015-2020")
...
```{r , echo=F, message = F}
state.one.var.1517 = read_csv("finalData/state-one-var-poisson-1517.csv")
...
```{r , echo=F, message = F}
state.one.var.1520 = read_csv("finalData/state-one-var-poisson-1520.csv")
...
state.random.poliexp = read_csv("finalData/state-police-exp-random.csv")
...
```{r , echo=F, message = F}
state.full.poisson = read_csv("finalData/state-full-poisson.csv")
state.full.poisson$`P-Value` = format(state.full.poisson$`P-Value`, digits = 3)
...
```{r , echo=F, message = F}
state.step.poisson = read_csv("finalData/state-step-poisson.csv")
state.step.poisson$`P-Value` = format(state.step.poisson$`P-Value`, digits = 3)
...
```{r , echo=F, message = F}
county.full.poisson = read_csv("finalData/county-full-poisson.csv")
county.full.poisson$`P-Value` = format(county.full.poisson$`P-Value`, digits = 3)
...
```{r , echo=F, message = F}
county.step.poisson = read_csv("finalData/county-step-poisson.csv")
county.step.poisson$`P-Value` = format(county.step.poisson$`P-Value`, digits = 3)
...
```{r , echo=F, message = F}
forecast.holtwinters = read_csv("finalData/HoltWinters-forecast.csv")
...
```{r , echo=F, message = F}
forecast.mlp = read_csv("finalData/MLP-forecast.csv")
...
Before we begin, we must get the datasets.
Here is the list of all the datasets we will use from the repository.

```{r,warning=FALSE, results = 'hide', message = F}
...
Once you run all of these datasets, we will now be able to proceed with creating the datasets for the models and visuals.
...
```{r, echo = T, results = 'hide', warning = F, fig.show='hide', message=F}
### Data Visual Plots
...
#### Topic Modeling Pt. 1 - Wordclouds
...
#### Topic Modeling Pt. 2 - LDA

```{r, echo = T, message= F, warning =F, result = 'hide', fig.show='none', fig.showtext= 'none'}
...
```{r, echo = T, results = 'hide', warning = F, fig.keep='none',fig.show = 'hide'}
...
```{r, echo = T, results = 'hide', warning = F, fig.show='hide'}
...
# Getting the table of the month, point forecast, low and high 85%, 95%
forecast(fit3,7)
...
# Getting the table of the month and point forecast
forecast(mlp2.fit,7)

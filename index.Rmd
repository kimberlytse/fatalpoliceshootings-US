...
author: "Arbaaz Mohideen & Kimberly Tse - \"Partners in Crime\" , and WITH GREAT REGARDS TO THE ALL MIGHTY research mentor Dr. Sayed Mostafa"
date: "7/31/2020"
output: 
  html_document:
    toc: true
...
## Acknowledgements
- NSA, NSF, grant. Need names and numbers. 


## Appendix

Here we will provide you the code and (both) the original and tidied datasets so that you can see the process of how we got our results. To get the files, simply click on the link to go to the repository where everything will be there for you.

### Data Wrangling, Cleaning and Processing 

After cleaning up the files, we have what we call "tidy data." ... Tidy data is what we need in order for us to commence data visualizations, modeling, and forecastings.

```{r,warning=FALSE, results = 'hide', message = F}
# These are the combined datasets that will be used for visuals and models. 
# NOTE: For the rest of the appendix, please refer to these tables to follow through
# what dataset we used in order to get the outcome
#
# You can find these files from the folder "finalData" in the repository
# if you want to replicate the data visuals.
...
Here is the code for all the visuals we created. For this section, please note that we will NOT be outputting the results here as the visuals are in the section already.
...
Any code for the "randomly selected" states?
